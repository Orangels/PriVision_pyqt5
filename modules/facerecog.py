...
class FaceRecog:
    def __init__(self, gpu_id=0):
        ...
        det_net = caffe.Net(cfg.FastFace.DEPLOY, cfg.FastFace.WEIGHTS, caffe.TEST)
        self.FastFace = Detector(det_net, mean=cfg.FastFace.PIXEL_MEANS, std=cfg.FastFace.PIXEL_STDS,
                                 scales=(300,), max_sizes=(400,), preN=500, postN=50, conf_thresh=0.5,
                                 color_map={0: [192, 0, 192], 1: [255, 64, 64]}, class_map=V.FACE_CLASS)

        id_net = caffe.Net(cfg.FaceID.DEPLOY, cfg.FaceID.WEIGHTS, caffe.TEST)
        self.I = Identity(id_net, mean=cfg.FaceID.PIXEL_MEANS, std=cfg.FaceID.PIXEL_STDS,
                          base_size=256, crop_size=224, crop_type='center', prob_layer='classifier',
                          gallery=np.load(cfg.FaceID.GALLERY))
        self.name_list = open(cfg.FaceID.GALLERY_NAMES).readlines()
