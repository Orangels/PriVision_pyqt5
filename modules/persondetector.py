...
class PersonDetector:
    def __init__(self, gpu_id=0, fastface=False):
        ...
        det_net = caffe.Net(cfg.PersonDet.DEPLOY, cfg.PersonDet.WEIGHTS, caffe.TEST)

        self.D = Detector(det_net, mean=cfg.PersonDet.PIXEL_MEANS, std=cfg.PersonDet.PIXEL_STDS,
                          scales=(480,), max_sizes=(800,), preN=500, postN=50, conf_thresh=0.7,
                          color_map={0: [192, 0, 192], 1: [64, 64, 255]}, class_map=V.PERSON_CLASS)

        if self.fastface:
            face_net = caffe.Net(cfg.FastFace.DEPLOY, cfg.FastFace.WEIGHTS, caffe.TEST)
            self.FastFace = Detector(face_net, mean=cfg.FastFace.PIXEL_MEANS, std=cfg.FastFace.PIXEL_STDS,
